...
Amphibians are often considered bioindicators because they generall
Habitat loss and fragmentation are some of the biggest threats to amphibian populations worldwide. 

Forestry and land-use changes often alter the physical characteristics of adjacent creeks and streams that make up amphibian habitat, potentially adversely impacting salmonid habitat. 

Monitoring efforts have been implemented to assess how activities like clear-cutting affect salmonid populations, including one by the 

Aquatic Vertebrate Population Study within the H.J. Andrews Forest Long Term Ecological Research Program. 

Restoration efforts are sometimes used to attempt to convert habitats from a degraded state back to their pre-disturbance condition, typically by creating pools or increasing habitat complexity. Understanding how the distribution of Pacific giant salamanders changes in different habitats can help guide restoration efforts. 

This report compares populations of Pacific giant salamanders observed in Mack Creek in Andrews Experimental Forest, Oregon. It compares the distribution of salamanders in different habitats in a section of forest that was clear-cut for logging in 1963 to a nearby 500 year old coniferous forest. 
...
# Subset data for each section for statistical testing
old_growth_weight <- salamander_weight %>% 
  filter(section == "OG")

clear_cut_weight <- salamander_weight %>% 
  filter(section == "CC")

salamander_weight_summary <- salamander %>% 
  filter(year == "2017") %>% 
  select(section, weight) %>% 
  group_by(section) %>% 
  summarize(
...
# The data are not normally distributed - will use a non-parametric test to compare ranks/medians as that will be a better metric of comparison (Mann-Whitney U)
weight_u <- wilcox.test(old_growth_weight$weight, clear_cut_weight$weight)
