...
salamander_habitat_2017 <- salamander %>% 
  ...
  filter(unittype != "IP") %>% 
  mutate(unittype = case_when(unittype == "C" ~ "Cascde",
                              unittype == "P" ~ "Pool",
                              unittype == "SC" ~ "Side-Channel"))
...
effect_size_habitat <- cohen.d(salamander_side_channel_2017$weight, salamander_pool_2017$weight)
```
