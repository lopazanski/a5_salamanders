...
![***Figure 3.*** Map of H. J. Andrews Experimental Forest with associated streams. Streams are depicted as blue lines and sample locations as yellow circles. Picture credit: Sashka Warner. ([source](https://www.researchgate.net/publication/324601050_Interactions_of_Roads_and_Deforestation_with_Stream_Sediment_Size_and_Network_Structure_in_the_Western_Cascade_Range_of_Oregon/figures?lo=1))](HJ_andrews_national_forest_map.png)
...
Trends in Pacific giant salamander abundance were compared for the clear cut and old growth sections of Mack Creek from 1993 to 2017 (Figure 4). 
...
ggplot(data = salamander_abundance, aes(x = year, y = abundance))+
  ...
  labs(x = "Year", # eader friendly labels
...
count_section_table <- salamander_count %>% 
  filter(id %in% c("1993", "2017")) %>% 
  pivot_wider(names_from = id, values_from = n) %>% 
  rename(y2017 = "2017") %>% 
  rename(y1993 = "1993") %>% 
  mutate(
    pct_change = ((y2017-y1993)/y1993)*100
  )
...
Throughout 1993-2017, Pacifc giant salamander abundances observed in sections of old growth forest were consistently higher than in historically clear-cut sections of the H. J. Andrews Experimental Forest, with exceptions to this trend in 1994 and 2014 (Figure 4). Both populations show a marked increase in abundance following 2001 and 2014, as well as a sharp decline in 2011. Overall, populations of salamanders increased from 1993 to 2017 - by 154% in the old growth section, and 192% in the clear cut section. 
...
# Alternative for non-parametric data would be the Mann-Whitney U (out of curiosity)
...
However, mean weights of Pacific giant salamanders differed significantly between pools and side channels (one-way ANOVA with post-hoc Tukey's HSD: F$_{`r aov_outputs[1]`,`r aov_outputs[2]`}$ = `r round(aov_outputs[7],2)`, *p* = `r round(aov_outputs[9],3)`). Mean weights for individuals from cascades did not differ significantly from the other two habitat types (Figure 5). The average weight for populations residing in pools (`r round(mean_pool, 2)` $\pm$ `r round(se_pool, 2)` g) was `r round(pct_diff, 2)`% more than that of populations observed in side-channels (`r round(mean_sc, 2)` $\pm$ `r round(se_side_channel, 2)` g). Due to large within group variation of salamander weights in each habitat type, an alternative metric -- population medians, for instance -- may lend to a more meaningful and accurate comparison across these groups. This could be achieved by ...?

***
### Summary

- From 1993-2017, Giant Pacific Salamnder abundances were consistently higher in sections of old growth forest than histrocially clear cut sections of forest, apart from two years (Figure 4) ***Not totally sure if I think this is the best summary point***
